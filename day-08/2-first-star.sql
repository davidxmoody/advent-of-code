...
CREATE OR REPLACE FUNCTION extend_table(start_pos int) RETURNS int LANGUAGE plpgsql AS $$
DECLARE
  i int := 0;
...
BEGIN
  ...
  INSERT INTO d8_nodes VALUES (start_pos, num_children, num_metadata, (SELECT array_agg(val ORDER BY pos) FROM d8 WHERE pos >= (start_pos + len_of_covered_children + 2) AND pos < (start_pos + len_of_covered_children + 2 + num_metadata)));
...
CREATE OR REPLACE FUNCTION build_table() RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
  DELETE FROM d8_nodes WHERE true;
  PERFORM extend_table(1);
END;
$$;
