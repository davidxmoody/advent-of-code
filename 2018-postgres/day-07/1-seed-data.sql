CREATE TABLE d7_raw (val TEXT NOT NULL);

INSERT INTO d7_raw VALUES
...
CREATE MATERIALIZED VIEW d7 AS SELECT
  substring(val FROM '^Step (.) must be finished before step . can begin\.$') AS pre,
  substring(val FROM '^Step . must be finished before step (.) can begin\.$') AS post
FROM d7_raw;
